      ...
      <table className="w-full">
        ...
        <tfoot>
          <tr className={`${headerBg} border-t-2 ${borderColor}`}>
            <td colSpan={10} className="py-4 px-4 text-[16px] font-bold text-[#0A0A0A] text-right rounded-bl-xl">施工费用结算合计：</td>
...
export function ConstructionDetails({ items, total }: { items: SettlementItem[]; total: number }) {
  return (
    <div className="bg-white rounded-[24px] shadow-card p-6 border border-[#E5E7EB] mb-8">
      <TableHeader colorClass="bg-[#8B5CF6]" title="匠心施工结算明细" icon={Hammer} />
      <DetailTable 
        items={items} 
        total={total} 
